...
let sha256 = (text) => {
  ...
  let message = new Utils(text);
  message.addShaPadding();
  let messageSchedule = [];
  for (let i = 0; i < 16; i++) {
    messageSchedule.push(message.bits.slice(i * 32, (i + 1) * 32));
  }
  ...
  for (let i = 16; i < 64; i++) {
    messageSchedule.push(
      Utils.addition(
        32,
        Utils.sigma1(messageSchedule[i - 2]),
        messageSchedule[i - 7],
        Utils.sigma0(messageSchedule[i - 15]),
  ...
  let finalValues = [...iInBits];

  for (let i = 0; i < messageSchedule.length; i++) {
    let t1 = Utils.addition(
      32,
      Utils.bigSigma1(finalValues[4]),
      Utils.choice(finalValues[4], finalValues[5], finalValues[6]),
      finalValues[7],
      kInBits[i],
      messageSchedule[i]
    );
    let t2 = Utils.addition(
      32,
      Utils.bigSigma0(finalValues[0]),
      Utils.majority(finalValues[0], finalValues[1], finalValues[2])
    );
