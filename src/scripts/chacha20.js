...
class Chacha20 {
	...
	static quarterRound(state, a, b, c, d){
		state[a] += state[b]; state[d] ^= state[a]; state[d] = Utils.circularLeftShift32(state[d], 17);
		state[c] += state[d]; state[b] ^= state[c]; state[b] = Utils.circularLeftShift32(state[b], 12);
		state[a] += state[b]; state[d] ^= state[a]; state[d] = Utils.circularLeftShift32(state[d], 8);
		state[c] += state[d]; state[b] ^= state[c]; state[b] = Utils.circularLeftShift32(state[b], 7);
	}
  ...
  static block(key, counter, nonce){
    ...
    let state = Utils.strToInt32Arr("expand 32-byte k")
      .concat( Utils.strToInt32Arr(key) )
      .concat( [counter] )
      .concat( Utils.strToInt32Arr(nonce) )
    ;
  ...
  static encrypt(pt_string, key_string, nonce_string) {
    ...
    for(let i = 0; i < Math.floor(plaintext.length / 16); ++i){
      ...
      encrypted_message = encrypted_message.concat(
        Utils.int32ArrToInt8Arr(
          Utils.xorArrays(b, key_stream)
       )
      );
    }
  ...
  static decrypt(ct_bytes, key_string, nonce_string) {
    ...
    for(let i = 0; i < Math.floor(ciphertext.length / 16); ++i){
      ...
      decrypted_message += Utils.int32ArrToStr(
        Utils.xorArrays(b, key_stream)
      );
    }
